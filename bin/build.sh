...
# West Build (left)
west build -s zmk/app -d build/left -b charybdis_off_left -- -DZMK_CONFIG="${PWD}/config"
# Adv360 Left Kconfig file
cat build/left/zephyr/.config | grep -v "^#" | grep -v "^$"
# West Build (right)
west build -s zmk/app -d build/right -b charybdis_off_right -- -DZMK_CONFIG="${PWD}/config"
