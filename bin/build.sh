...
# West Build (left)
west build -s zmk/app -d build/left -b nice_nano_v2 -- -DSHIELD=charybdis1_left -DZMK_CONFIG="${PWD}/config"
# Adv360 Left Kconfig file
cat build/left/zephyr/.config | grep -v "^#" | grep -v "^$"
# West Build (right)
west build -s zmk/app -d build/right -b nice_nano_v2 -- -DSHIELD=charybdis1_right -DZMK_CONFIG="${PWD}/config"
